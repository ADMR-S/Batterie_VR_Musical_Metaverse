class XRDrum{
    ...
    createDrumComponent(name, diameter, height, coordinates, midiKey){//Créer les percussions à peau (snare, kick, tom, etc.)
        const drumContainer = new BABYLON.TransformNode(name + "Container", this.scene);
        ...
        triggerAggregate.body.setEventMask(this.eventMask);

        this.hk.onTriggerCollisionObservable.add((collision) => {
            if (collision.type === "TRIGGER_ENTERED" && collision.collidedAgainst.transformNode.id === "snareTrigger") {
                console.log('snare trigger entered', collision);
        ...
        */
        ...
        drumContainer.position = coordinates;
