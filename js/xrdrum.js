...
class XRDrum{
    ...
    createDrumComponentTrigger(name, diameter, height, drumComponentContainer){//Créer les peaux des percussions à peau (snare, kick, tom, etc.)
        ...
        const trigger = BABYLON.MeshBuilder.CreateCylinder(name + "Trigger", { diameter: diameter, height: height }, this.scene);
        trigger.position = new BABYLON.Vector3(0, height-height/2, 0); //0.05 = trigger height / 2 pour aligner parfaitement avec le body
    ...
    createDrumComponent(name, diameter, height, coordinates){//Créer les percussions à peau (snare, kick, tom, etc.)
        ...
        this.createDrumComponentTrigger(name, diameter, 0.1, drumComponentContainer);
