...
var createScene = async function () {
    ...
    stick1.position = new BABYLON.Vector3(0,5, 4, 0);
    stick2.position = new BABYLON.Vector3(-0,5, 4, 0);
    physicsBodies.push(stick1);
    physicsBodies.push(stick2);
    ...
    const observer = sphereObservable.add((collisionEvent) => {
    ...
    xr.input.onControllerAddedObservable.add((controller) => {
        controller.onMotionControllerInitObservable.add((motionController) => {
            motionController.onModelLoadedObservable.add(mc => {
                ...
                controllerAggregate.body.setMotionType(BABYLON.PhysicsMotionType.ANIMATED); // Set motion type to KINEMATIC
                ...
                const pickStick = (stickAggregate) => {
                    ...
                    const pickInfo = scene.pickWithRay(pickRay, (mesh) => {});
                    if (pickInfo.hit) {
