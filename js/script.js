...
var createScene = async function () {
    ...
    const drum = new XRDrum(audioContext, scene, eventMask, xr);
        ...
        controller.onMotionControllerInitObservable.add((motionController) => {
            motionController.onModelLoadedObservable.add(mc => {
                ...
                */
                ...
                /*
                // Add ray to visualize selection
                const ray = new BABYLON.Ray(controller.pointer.position, controller.pointer.forward, 10);
                const rayHelper = new BABYLON.RayHelper(ray);
                rayHelper.show(scene);

                controller.onFrameObservable.add(() => {
                    ray.origin = controller.pointer.position;
                    ray.direction = controller.pointer.forward;
                    rayHelper.update(ray);
                });
                */

                /*
                //RAY
                const motionControllerComponentIds = motionController.getComponentIds();
                const triggerComponent = motionController.getComponent(motionControllerComponentIds[0]);
                triggerComponent.onButtonStateChangedObservable.add((component) => {
                    if (component.pressed && component.value > 0.8) {
                        const resultRay = this.createRayFromController(controller);
                        const raycastHit = this.scene.pickWithRay(resultRay);
